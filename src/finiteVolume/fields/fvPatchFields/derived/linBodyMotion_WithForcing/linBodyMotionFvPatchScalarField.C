...
:
    ...
    accelRelaxation_(1),
    accelDamping_(1),
    prescribedMotion_(false),
    prescribedAmp_(scalar(0)),
    prescribedPhase_(scalar(0)),
    prescribedOmega_(0),
...
:
    ...
    prescribedMotion_(ptf.prescribedMotion_),
    prescribedAmp_(ptf.prescribedAmp_),
    prescribedPhase_(ptf.prescribedPhase_),
    prescribedOmega_(ptf.prescribedOmega_),
...
void Foam::linBodyMotionFvPatchScalarField::readProperties()
{
    ...
    // Prescribed motion.  Read last: the default forcing frequency is the
    // encounter frequency, which is only known once the wave data above have
    // been processed.
    prescribedMotion_ =
        bodyDict.getOrDefault<Switch>("prescribedMotion", Switch(false));

    if (prescribedMotion_)
    {
        const auto readSix = [&](const word& key, FixedList<scalar, 6>& dst)
        {
            const scalarList v(bodyDict.get<scalarList>(key));
            if (v.size() != 6)
            {
                FatalIOErrorInFunction(bodyDict)
                    << key << " needs 6 entries (surge sway heave roll pitch"
                       " yaw), got " << v.size() << exit(FatalIOError);
            }
            forAll(dst, i) dst[i] = v[i];
        };

        readSix("prescribedAmplitude", prescribedAmp_);

        if (bodyDict.found("prescribedPhase"))
        {
            readSix("prescribedPhase", prescribedPhase_);
        }

        prescribedOmega_ =
            bodyDict.getOrDefault<scalar>("prescribedOmega", omegaE_);

        if (fixBody_)
        {
            FatalIOErrorInFunction(bodyDict)
                << "prescribedMotion and fixBody are both set; they contradict"
                   " each other" << exit(FatalIOError);
        }

        if (Pstream::master())
        {
            Info<< "    prescribed motion at omega = " << prescribedOmega_
                << " rad/s (encounter frequency " << omegaE_ << ")" << nl
                << "      amplitudes";
            forAll(prescribedAmp_, i) Info<< ' ' << prescribedAmp_[i];
            Info<< endl;
        }
    }
}
...
void Foam::linBodyMotionFvPatchScalarField::prescribeMotion(const scalar t)
{
    const scalar w = prescribedOmega_;

    // Ramp and its first two derivatives.  Differentiating the ramp rather
    // than applying it to q alone keeps q, qDot and qDdot an exactly
    // consistent triple, so the body condition (which uses qDot) and the
    // pressure (which responds to qDdot) see the same motion.
    scalar r = 1.0, rDot = 0.0, rDdot = 0.0;
    if (rampTime_ > SMALL && t < rampTime_)
    {
        const scalar a = constant::mathematical::pi/rampTime_;
        r     = 0.5*(1.0 - Foam::cos(a*t));
        rDot  = 0.5*a*Foam::sin(a*t);
        rDdot = 0.5*a*a*Foam::cos(a*t);
    }

    forAll(qNew_, i)
    {
        const scalar amp = prescribedAmp_[i];
        const scalar c = Foam::cos(w*t + prescribedPhase_[i]);
        const scalar s = Foam::sin(w*t + prescribedPhase_[i]);

        qNew_[i]     = amp*(r*c);
        qDotNew_[i]  = amp*(rDot*c - r*w*s);
        qDdotNew_[i] = amp*(rDdot*c - 2.0*rDot*w*s - r*w*w*c);
    }
}
