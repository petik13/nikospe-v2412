...
{
    defineTypeNameAndDebug(myMeanForce, 0);
    addToRunTimeSelectionTable(functionObject, myMeanForce, dictionary);
}
}
...
void Foam::functionObjects::myMeanForce::setCoordinateSystem
(
...
Foam::volVectorField& Foam::functionObjects::myMeanForce::force()
{
...
Foam::volVectorField& Foam::functionObjects::myMeanForce::moment()
{
...
void Foam::functionObjects::myMeanForce::initialise()
{
    if (initialised_)
    {
        return;
    ...
    else
    {
        ...
        {
            FatalErrorInFunction
                << "Could TUrgut not find rho:" << rhoName_ << " in database"
                << exit(FatalError);
        }
    }
...
void Foam::functionObjects::myMeanForce::reset()
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::myMeanForce::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::myMeanForce::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::functionObjects::myMeanForce::rho(const label patchi) const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::myMeanForce::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::myMeanForce::addToPatchFields
(
    const label patchi,
...
void Foam::functionObjects::myMeanForce::addToInternalField
(
...
void Foam::functionObjects::myMeanForce::createIntegratedDataFiles()
{
    if (!forceFilePtr_)
    {
...
void Foam::functionObjects::myMeanForce::writeIntegratedDataFileHeader
(
    const word& header,
    OFstream& os
) const
...
void Foam::functionObjects::myMeanForce::writeIntegratedDataFiles()
{
    const auto& coordSys = coordSysPtr_();
...
void Foam::functionObjects::myMeanForce::writeIntegratedDataFile
(
    const vector& pres,
    const vector& vis,
...
void Foam::functionObjects::myMeanForce::logIntegratedData
(
...
Foam::functionObjects::myMeanForce::myMeanForce
(
    const word& name,
    const Time& runTime,
    const dictionary& dict,
...
bool Foam::functionObjects::myMeanForce::read(const dictionary& dict)
{
    ...
    else
    {
        ...
        if (rhoName_ == "rhoInf")
        {
            rhoRef_ = 1;// dict.getCheck<scalar>("rhoInf", scalarMinMax::ge(SMALL));
...
void Foam::functionObjects::myMeanForce::calcForcesMoments()
{
    initialise();

    reset();
	
	findFreeSurfaceFaces(); 
	readZetaValues();
    ...
    else
    {
		...
		//const vector gVec = gField.value();
		const scalar gMag = 9.81;
        ...
        for (const label patchi : patchIDs_)
        {
            ...
            const vectorField fP(rhoRef*Sfb[patchi]*(pb[patchi] - pRef));
			 ...
			 // Access zeta values already read in readZetaValues()
			const List<scalar>& zetaVals = zetaAtBodyFaces_[patchi];

			// Access face mapping and edge lengths from findFreeSurfaceFaces()
			const List<labelPair>& facePairs = bodyToFreeSurfaceFaces_[patchi];
			const List<scalar>& edgeLens = bodyToEdgeLengths_[patchi];

			vectorField waveForces(fP.size(), vector::zero);

			
			forAll(facePairs, k)
			{
				const label bodyLocalFaceI = facePairs[k].first();
				
				// Face area vector and normal
				const vector Sf_body = Sfb[patchi][bodyLocalFaceI];
				const vector n_body = Sf_body/mag(Sf_body);

				// Hydrostatic head relative to mean free surface
				const scalar zeta = zetaVals[k];           // instantaneous free-surface elevation at that edge
				const scalar Ledge = edgeLens[k];          // shared edge length
				
				// Simple linear hydrostatic pressure variation along vertical
				scalar pWave = 0.5*rhoRef*gMag*zeta*zeta;

				// Local distributed line force (approximate integration along edge)
				waveForces[bodyLocalFaceI] += pWave * Ledge * n_body;
			}

            addToPatchFields(patchi, Md, fP, waveForces);
        }
    }
...
Foam::vector Foam::functionObjects::myMeanForce::forceEff() const
{
    return sumPatchForcesP_ + sumPatchForcesV_ + sumInternalForces_;
}


Foam::vector Foam::functionObjects::myMeanForce::momentEff() const
{
    return sumPatchMomentsP_ + sumPatchMomentsV_ + sumInternalMoments_;
}


bool Foam::functionObjects::myMeanForce::execute()
{
    calcForcesMoments();
    ...
    logIntegratedData("myMeanForce", localFp, localFv, localFi);
...
bool Foam::functionObjects::myMeanForce::write()
{
    if (writeToFile())
    {
