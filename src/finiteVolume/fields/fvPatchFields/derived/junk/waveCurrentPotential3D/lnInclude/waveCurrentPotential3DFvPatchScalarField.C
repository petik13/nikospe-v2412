...
void Foam::waveCurrentPotential3DFvPatchScalarField::updateCoeffs()
{
		...
		if (std::fabs(U0) > SMALL) // if current speed is non-zero
		{
			...
			if (neigboursCalculated_) // set in calcNeigboursV3
			{
				...
				UPFDV2();
                    ...
                    {
                        scalar maxPhiDx = gMax(mag(PhiDx_));
                        scalar sumPhiDx = gSum(mag(PhiDx_));
                        scalar maxPhiDy = gMax(mag(PhiDy_));
                        scalar sumPhiDy = gSum(mag(PhiDy_));
                        
                        // Zeta derivatives stats
                        scalar maxZetaDx = gMax(mag(zetaDx_.component(vector::X)));
                        scalar sumZetaDx = gSum(mag(zetaDx_.component(vector::X)));
                        scalar maxZetaDy = gMax(mag(zetaDx_.component(vector::Y)));
                        scalar sumZetaDy = gSum(mag(zetaDx_.component(vector::Y)));

                        // Field stats
                        scalar maxPhi = gMax(mag(Phi0Patch));
                        scalar maxZeta = gMax(mag(zetap));

                        label totalFaces = returnReduce(PhiDx_.size(), sumOp<label>());
                        
                        Info << "Derivatives stats (Time=" << db().time().value() << "):" << nl
                             << "  PhiDx: max=" << maxPhiDx << ", mean=" << (totalFaces > 0 ? sumPhiDx/totalFaces : 0) << nl
                             << "  PhiDy: max=" << maxPhiDy << ", mean=" << (totalFaces > 0 ? sumPhiDy/totalFaces : 0) << nl
                             << "  ZetaDx: max=" << maxZetaDx << ", mean=" << (totalFaces > 0 ? sumZetaDx/totalFaces : 0) << nl
                             << "  ZetaDy: max=" << maxZetaDy << ", mean=" << (totalFaces > 0 ? sumZetaDy/totalFaces : 0) << nl
                             << "  Field Phi: max=" << maxPhi << nl
                             << "  Field Zeta: max=" << maxZeta << endl;
                    }
    ...
    scalar globalMaxZeta = gMax(zetap & nfRef);
	if (globalMaxZeta > 100.0)
    {
        // Find local max to identify location
        scalar maxVal = -GREAT;
        label maxIdx = -1;
        
        forAll(zetap, i)
        {
            // Replicate the masking logic if nfRef is a mask
            scalar val = zetap[i] & nfRef[i];
            if (val > maxVal)
            {
                maxVal = val;
                maxIdx = i;
        ...
        if (maxIdx != -1)
        {
             const vector& C = patch().Cf()[maxIdx];
             Info << "Local Max zetap found at local face " << maxIdx 
                  << " at position " << C << " value: " << maxVal 
                  << " (nfRef: " << nfRef[maxIdx] << ")" << endl;
                  
             // Check if it is near inlet or body
             bool isBody = false;
             bool isInlet = false;
             if (ownerHasBodyFace_.size() > maxIdx && ownerHasBodyFace_[maxIdx]) isBody = true;
             if (ownerHasInletFace_.size() > maxIdx && ownerHasInletFace_[maxIdx]) isInlet = true;
             
             Info << "  Is Body: " << isBody << " Is Inlet: " << isInlet << endl;
             Info << "  SchemeX: " << (schemeCodeX_.size() > maxIdx ? schemeCodeX_[maxIdx] : -999) 
                  << " SchemeY: " << (schemeCodeY_.size() > maxIdx ? schemeCodeY_[maxIdx] : -999) << endl;
        }

      FatalErrorInFunction
          << "Surface elevation (zetap) exceeded limit of 100 at time = "
          << db().time().value() << nl
          << "  max(zetap) = " << globalMaxZeta << nl
          << abort(FatalError);
  }
