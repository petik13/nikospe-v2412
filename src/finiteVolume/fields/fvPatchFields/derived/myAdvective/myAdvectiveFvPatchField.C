...
{
    // Lookup the *vol* field U on this patch
    const auto& Up =
        this->patch().template lookupPatchField<volVectorField>(phiName_);
        // phiName_ == "U" from the dictionary

    // Up is an fvPatchField<vector>. The dot-product '&' gives a scalarField
    const Foam::scalarField phip(Up & this->patch().Sf());

    // Face-normal speed = flux / |Sf|
