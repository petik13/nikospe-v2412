...
:
    ...
    mTerms_(true),
    mTermsFromSurface_(true),
    mTermFeatureAngle_(50),
    nGradW_(),
    nGradWValid_(false),
...
:
    ...
    mTerms_(ptf.mTerms_),
    mTermsFromSurface_(ptf.mTermsFromSurface_),
    mTermFeatureAngle_(ptf.mTermFeatureAngle_),
    nGradW_(),
    nGradWValid_(false),
...
void Foam::linBodyMotionFvPatchScalarField::readProperties()
{
    ...
    // m-terms.  mTermsFromSurface picks the accurate in-surface evaluation of
    // (n.grad)W over the legacy volume-gradient one; see the header for why
    // the legacy route does not converge.
    mTerms_ = bodyDict.getOrDefault<Switch>("mTerms", Switch(true));
    mTermsFromSurface_ =
        bodyDict.getOrDefault<Switch>("mTermsFromSurface", Switch(true));
    mTermFeatureAngle_ =
        bodyDict.getOrDefault<scalar>("mTermFeatureAngle", 50);
...
//- (n.grad)W at one face from the volume gradient of Us.  Kept for A/B only:
//  it is zeroth-order accurate, see the header.
Foam::vector Foam::linBodyMotionFvPatchScalarField::nGradWLegacy
(
    const label i,
    const vector& n,
    const tensorField& gradWb,
    const tensorField& gradWc
) const
{
    const vector vb(gradWb[i].T() & n);   // == snGrad(Us), pure normal
    const vector vc(gradWc[i].T() & n);   // cell value, carries the tangent

    return (vb & n)*n + (vc - (vc & n)*n);
}


void Foam::linBodyMotionFvPatchScalarField::publishNGradW() const
{
    auto& mesh = const_cast<fvMesh&>(patch().boundaryMesh().mesh());

    auto* fp = mesh.getObjectPtr<volVectorField>("nGradW");

    if (!fp)
    {
        fp = new volVectorField
        (
            IOobject
            (
                "nGradW",
            ...
            mesh,
            dimensionedVector(dimless/dimTime, Zero)
        );
        fp->store();
    }

    fp->boundaryFieldRef()[patch().index()] == nGradW_;
}


void Foam::linBodyMotionFvPatchScalarField::calcNGradW()
{
    if (nGradWValid_) return;
    nGradWValid_ = true;

    const fvPatch& fvp = patch();
    const label nLocal = fvp.size();
    const label patchi = fvp.index();

    nGradW_.setSize(nLocal, Zero);

    const auto& Us = db().lookupObject<volVectorField>(UsName_);
    const auto& gradUs = db().lookupObject<volTensorField>(gradUsName_);

    const vectorField nLoc(fvp.nf());
    const vectorField& WLoc = Us.boundaryField()[patchi];

    // --- legacy route, or the fallback for faces with too small a stencil ---
    const tensorField& gradWb = gradUs.boundaryField()[patchi];
    const tensorField gradWc(gradUs.boundaryField()[patchi].patchInternalField());
    ...
    if (!mTermsFromSurface_)
    {
        if (Pstream::master())
        {
            Info<< "    m-terms: (n.grad)W from the volume gradient of "
                << UsName_ << " (legacy, zeroth order)" << endl;
        }
        publishNGradW();
        return;
    }
    ...
    // A snapped facet normal wobbles face to face.  The patch point normals
    // average over each point's whole face ring, and the tangential m-term IS
    // the surface curvature, so the normal field is exactly what has to be
    // smooth for it to mean anything.
    const scalarField hLoc(Foam::sqrt(fvp.magSf()));
    vectorField nSm(nLocal, Zero);
    {
        const polyPatch& pp = fvp.patch();
        const vectorField& pn = pp.pointNormals();
        const faceList& lf = pp.localFaces();

        forAll(lf, i)
        {
            vector v(Zero);
            for (const label pi : lf[i]) v += pn[pi];
            v /= (mag(v) + VSMALL);
            nSm[i] = ((v & nLoc[i]) < 0 ? -v : v);
        }
    }

    // --- gather the whole hull patch onto every rank ------------------------
    // A body patch is O(1e3-1e4) faces, so this is a few hundred kB, once.  It
    // removes the processor-boundary problem outright: every face gets the
    // stencil it would have had in serial.
    ...
    // --- weighted plane fit  f = c0 + c1 u + c2 v,  returns (c1, c2) --------
    // u, v are non-dimensionalised by the local face size so the normal matrix
    // is well scaled and a single determinant tolerance is meaningful.
    auto fitGrad = []
    (
        const UList<scalar>& u,
        const UList<scalar>& v,
        const UList<scalar>& f,
        scalar& dfdu,
        scalar& dfdv
    ) -> bool
    {
        scalar a00=0, a01=0, a02=0, a11=0, a12=0, a22=0, b0=0, b1=0, b2=0;

        forAll(u, k)
        {
            a00 += 1;         a01 += u[k];        a02 += v[k];
            a11 += u[k]*u[k]; a12 += u[k]*v[k];   a22 += v[k]*v[k];
            b0  += f[k];      b1  += u[k]*f[k];   b2  += v[k]*f[k];
        }

        const symmTensor A(a00, a01, a02, a11, a12, a22);
        if (mag(det(A)) < 1e-8*max(a00*a11*a22, SMALL)) return false;

        const vector c(inv(A) & vector(b0, b1, b2));
        dfdu = c.y();
        dfdv = c.z();
        return true;
    };

    // --- per-face surface operators ----------------------------------------
    const scalar cosLim = Foam::cos(degToRad(mTermFeatureAngle_));
    ...
    for (label i = 0; i < nLocal; ++i)
    {
        const vector& ni = nSm[i];
        const vector& ci = allCf[Pstream::myProcNo()][i];
        const scalar  hi = hLoc[i];

        // local tangent frame
        vector t1(mag(ni.x()) < 0.9 ? vector(1,0,0) : vector(0,1,0));
        t1 -= (t1 & ni)*ni;
        t1 /= (mag(t1) + VSMALL);
        const vector t2(ni ^ t1);

        bool built = false;

        for (scalar R = 3.5*hi; R < 12.1*hi; R *= 1.6)
        {
            uu.clear(); vv.clear();
            ...
            for (label j = 0; j < nGlob; ++j)
            {
                const vector d(gCf[j] - ci);
                if (magSqr(d) > R2) continue;
                if ((gN[j] & ni) < cosLim) continue;   // across a feature

                uu.append((d & t1)/hi);
                vv.append((d & t2)/hi);

                const vector dn(gN[j] - ni);
                nx.append(dn.x()); ny.append(dn.y()); nz.append(dn.z());
        ...
        if (!built) { ++nFallback; continue; }

        // curvature: K = grad_s n
        vector dndu(Zero), dndv(Zero);
        scalar pu = 0, pv = 0;
        bool ok = true;
        ok = ok && fitGrad(uu, vv, nx, pu, pv);  dndu.x() = pu; dndv.x() = pv;
        ok = ok && fitGrad(uu, vv, ny, pu, pv);  dndu.y() = pu; dndv.y() = pv;
        ok = ok && fitGrad(uu, vv, nz, pu, pv);  dndu.z() = pu; dndv.z() = pv;

        // surface divergence of the tangential velocity
        scalar dW1du = 0, dW1dv = 0, dW2du = 0, dW2dv = 0;
        ok = ok && fitGrad(uu, vv, w1, dW1du, dW1dv);
        ok = ok && fitGrad(uu, vv, w2, dW2du, dW2dv);

        if (!ok) { ++nFallback; continue; }

        const scalar rh = 1.0/hi;               // undo the non-dimensionalisation

        // Shape operator in the (t1,t2) basis.  It is symmetric for a smooth
        // surface, so symmetrising costs nothing and removes error.
        scalar S11 = (dndu & t1)*rh;
        scalar S22 = (dndv & t2)*rh;
        const scalar S12 = 0.5*(((dndv & t1) + (dndu & t2))*rh);

        vector Wt(WLoc[i]);
        Wt -= (Wt & ni)*ni;
        const scalar Wu = Wt & t1;
        const scalar Wv = Wt & t2;

        // [(n.grad)W]_t = -K W
        const vector tang(-(S11*Wu + S12*Wv)*t1 - (S12*Wu + S22*Wv)*t2);

        // n.(n.grad)W = -div_s W_t
    ...
    const label nTot = returnReduce(nLocal, sumOp<label>());

    if (Pstream::master())
    {
        Info<< "    m-terms: (n.grad)W from in-surface least-squares operators"
            << " on " << nTot - nFallback << '/' << nTot << " faces";
        if (nFallback)
        {
            Info<< ", " << nFallback << " fell back to the volume gradient";
        }
        Info<< endl;
    }
...
void Foam::linBodyMotionFvPatchScalarField::solveMotion()
{
    ...
    if (Pstream::master())
    {
        Info<< "    qddot corrector change percentage =";
        forAll(qDdotNew_, i)
            Info<< ' ' << mag(qDdotNew_[i] - qDdotPrevIter_[i])/
                (mag(qDdotPrevIter_[i]) + VSMALL)*100.0;
        Info<< endl;
    }
...
void Foam::linBodyMotionFvPatchScalarField::updateCoeffs()
{
    ...
    // (n.grad)W on the hull.  W, the hull and its curvature are all steady,
    // so this is built once, on the first step, after the solver has finished
    // the basis flow.
    calcNGradW();
    ...
    const scalar mOn = (mTerms_ ? 1.0 : 0.0);
